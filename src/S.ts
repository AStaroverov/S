...
var S = <S>function S<T>(fn: (v: T | undefined) => T, value?: T): () => T {
    ...
    const { node, value: _value } = makeComputationNode(
        fn,
        value,
        false,
        false,
...
S.useValue = function localValue<T>(
    current: T,
    eq?: (a: T, b: T) => boolean,
): DataSignal<T> {
    let age = -1;
    let node = Owner!.localValue[Owner!.localValueIndex++] as DataNode;

    if (!node) {
        node = Owner!.localValue[Owner!.localValueIndex - 1] = new DataNode(
            current,
        );
    }
...
S.useEffect = function useEffect<T>(
    depsOrCb: unknown[] | ((v: T | undefined) => T),
    cb?: (v: T | undefined) => T,
): void {
    if (!Owner) {
        throw new Error('Can use useEffect only inside computed');
    }

    const fn = (cb ? cb : depsOrCb) as (v: T | undefined) => T;
    const deps = (cb ? depsOrCb : DEFAULT_DEPS) as unknown[];
    const prevDeps = Owner!.localValue[Owner!.localValueIndex++] as unknown[];

    if (!shallowEqual(prevDeps, deps)) {
        makeComputationNode(fn, undefined, false, false);

        Owner!.localValue[Owner!.localValueIndex - 1] = deps;
    }
};

S.async = function async(cb: (setter: <T>(cb: () => T) => T) => void): void {
    const owner = Owner;
    const listener = Listener;
    const setter = <T>(cb: () => T) => {
        const o = Owner;
...
class ComputationNode {
    ...
    localValueIndex = 0;
    localValue: (unknown | unknown[] | DataNode | ComputationNode)[] = [];

    constructor() {
        console.log('ComputationNode');
    }
...
function shallowEqual(
    objA: unknown | unknown[],
    objB: unknown | unknown[],
): boolean {
